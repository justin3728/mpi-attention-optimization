#define USE_MPI
#ifndef KV_BCAST_THRESHOLD
#define KV_BCAST_THRESHOLD (64u * 1024u * 1024u)
#endif
#include <math.h>
#include <mpi.h>
#include <stdbool.h>
...
void attention(double* Q, double* K, double* V, double* result,
    ...
    const int B = 512;  // Query rows per pipeline batch

    // Alloc shared buffers
    float *K_local = checked_malloc(sizeof(float)*n_local*dk);
    float *V_local = checked_malloc(sizeof(float)*n_local*dv);
    float *lmax = checked_malloc(sizeof(float)*B), *lsum = checked_malloc(sizeof(float)*B);
    float *gmax = checked_malloc(sizeof(float)*B), *gsum = checked_malloc(sizeof(float)*B);

    float scale = 1.0f / sqrtf((float)dk);

    // Adaptive strategy: choose between Bcast and Scatterv based on data size
    // Threshold: if total data < 64 MiB, use Bcast (faster for small data)
    // Otherwise use Scatterv (more memory efficient for large data)
    size_t total_kv_size = (size_t)n * (dk + dv) * sizeof(float);
    const size_t BCAST_THRESHOLD = KV_BCAST_THRESHOLD;  // 64 MiB
    ...
    if (use_bcast) {
        ...
        if (mpi_rank == root) {
            Kf = checked_malloc(sizeof(float)*n*dk);
            Vf = checked_malloc(sizeof(float)*n*dv);
        ...
        if (mpi_rank == root) {
            float *Kf = checked_malloc(sizeof(float)*n*dk);
            float *Vf = checked_malloc(sizeof(float)*n*dv);

            cvt_d2f_avx512(Kf, K, (size_t)n*dk);
            cvt_d2f_avx512(Vf, V, (size_t)n*dv);

            int *ck = checked_malloc(sizeof(int)*mpi_size), *dk_d = checked_malloc(sizeof(int)*mpi_size);
            int *cv = checked_malloc(sizeof(int)*mpi_size), *dv_d = checked_malloc(sizeof(int)*mpi_size);
    ...
    float *q_bufs = checked_malloc(num_q_buf * B * dk * sizeof(float));
    float **q_batch_ptrs = checked_malloc(num_q_buf * sizeof(float *));
    ...
    const int num_buf = 2;
    float *cbs = checked_malloc(num_buf * B * dv * sizeof(float));
    float **cb_bufs = checked_malloc(num_buf * sizeof(float *));
    ...
    float **rb_bufs = NULL;
    if (mpi_rank == root) {
        rbs = checked_malloc(num_buf * B * dv * sizeof(float));
        rb_bufs = checked_malloc(num_buf * sizeof(float *));
        for (int k = 0; k < num_buf; ++k) {
...
int main(int argc, char* argv[]) {
    ...
    int ok = 1;
    if (rank == 0) {
        ok = verify(argv[1], result, m, n, dk, dv);
        if (ok) {
            printf("Correct!\nElapsed time: %.2lf us\n", duration_max * 1e6);
        } else {
            puts("Wrong!");
